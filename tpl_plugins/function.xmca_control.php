 ...
 * @return string 
 */
function smarty_function_xmca_control($args) {
	...
	if (array_key_exists("confirm", $args)) {
		$action = "xmca.confirm('" . str_replace("'", "\\'", @$args["confirmTitle"]) . "', '" . str_replace("'", "\\'", @$args["confirmQuestion"]) . "', $jsArgs); return false;";
	} else {
		$action = "xmca.request($jsArgs); return false;";
	}

	switch (!empty($image) ? "image" : (\array_key_exists("style", $args) ? $args["style"] : "button")) {
		case "image":
			return '<a href="javascript:' . $action . '" title="' . $title . '" class="' . $class . '"><img src="img/layout/controls/' . $image . '"/></a>';
			break;
		
		case "link":
			return '<a href="javascript:' . $action . '" title="' . $title . '" class="' . $class . '"><span>' . $title . '</span></a>';
			break;
		
		case "button":
		default:
			return '<button onclick="' . $action . '" class="' . $class . '">' . $title . '</button>';
			break;
	}
