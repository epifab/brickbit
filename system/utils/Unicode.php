 ...
 */
function cb_xml_parser_create(&$data) {
  ...
  if (!in_array(strtolower($encoding), $php_supported)) {
    $out = cb_convert_to_utf8($data, $encoding);
    if ($out !== FALSE) {
      $encoding = 'utf-8';
 ...
 */
function cb_convert_to_utf8($data, $encoding) {
 ...
 */
function cb_truncate_bytes($string, $len) {
...
function truncate_utf8($string, $max_length, $wordsafe = FALSE, $add_ellipsis = FALSE, $min_wordsafe_length = 1) {
  ...
  if (cb_strlen($string) <= $max_length) {
  ...
  if ($add_ellipsis) {
    ...
    $ellipsis = cb_substr(t('...'), 0, $max_length);
    $max_length -= cb_strlen($ellipsis);
    $max_length = max($max_length, 0);
  }
  ...
  if ($wordsafe) {
    ...
    if ($found) {
      ...
      $string = cb_substr($string, 0, $max_length);
    }
  }
  else {
    $string = cb_substr($string, 0, $max_length);
...
function mime_header_encode($string) {
  if (preg_match('/[^\x20-\x7E]/', $string)) {
    ...
    $output = '';
    while ($len > 0) {
      $chunk = cb_truncate_bytes($string, $chunk_size);
...
function _mime_header_decode($matches) {
  ...
  if (strtolower($matches[1]) != 'utf-8') {
    $data = cb_convert_to_utf8($data, $matches[1]);
  }
  return $data;
}
 ...
 * @ingroup php_wrappers
 */
function cb_ucfirst($text) {
  // Note: no mbstring equivalent!
  return cb_strtoupper(cb_substr($text, 0, 1)) . cb_substr($text, 1);
}
 ...
 * @ingroup php_wrappers
 */
function cb_substr($text, $start, $length = NULL) {
  global $multibyte;
  if ($multibyte == UNICODE_MULTIBYTE) {
